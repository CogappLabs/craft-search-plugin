...
(() => {
  ...
  testBtn.addEventListener('click', function () {
    const resultEl = document.getElementById('test-connection-result') as HTMLElement;
    ...
    Craft.sendActionRequest('POST', 'search-index/indexes/test-connection', {
      data: { engineType, engineConfig },
    })
      .then((response) => {
        testBtn.classList.remove('loading');
        resultEl.textContent = response.data.message as string;
  ...
  const sourcesSection = document.getElementById('synced-sources') as HTMLElement;
